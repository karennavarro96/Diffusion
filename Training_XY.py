...
#model.add(tensorflow.keras.layers.Convolution2D(filters= 4, kernel_size =(3,3), strides= (1,1), padding='same', name='conv1')) #96


model.add(tensorflow.keras.layers.Convolution2D(filters= 4, kernel_size =(2,2), strides= (1,1), padding='same', name='conv1')) #96
...
model.add(Dense(15, name='dense2'))  #1024
#model.add(tensorflow.keras.layers.Dropout(0.5))
...
model.compile(optimizer=tensorflow.keras.optimizers.Adam(lr=0.001), loss='mean_squared_error')
...
model.save_weights(cur_dir+ "ModelXY.h5")
model_json = model.to_json()
with open(cur_dir+ "ModelXY.json", "w") as json_file:
